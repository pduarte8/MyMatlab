...
rdir = ['/tos-project3/NS9081K/NORSTORE_OSL_DISK/NS9081K/K160_bgc/ThirdRoundOfSimulations/Sim28/'];
...
% Read positions
pfil = '/cluster/home/pduarte/matlabmfiles/K160/Mooring_overview.txt';
inp = load(pfil);
...
for p=1:length(lon)
  ...
  t0_s = date2unixsecs(yy(p),mm(p),dd(p));
  t1_s = t0_s + 24*3600;
  [Year Month Day] = unixsecs2date(t1_s);
  %t1 = yy(p)*1000000 + mm(p)*10000 + (dd(p)+1)*100;
  t1 = Year*1000000 + Month*10000 + Day*100;
  ...
  for k=1:N
    ...
    %clear F
    saltp(k) = 0.5*(salt3d(x_rho1,eta_rho,k) + salt3d(x_rho2,eta_rho,k));
    tempp(k) = 0.5*(temp3d(x_rho1,eta_rho,k) + temp3d(x_rho2,eta_rho,k));
  end
  ...
  if ~isnan(saltp(end))
    ...
    totdepth = 0.5*(h(x_rho1,eta_rho)+h(x_rho2,eta_rho));
    clear F
