...
for i=1:length(lon)
  ...
  obs = load(['Data/ctd_',num2str(afile(i),'%04d'),'.asc']);
  zlev_unsorted = obs(:,1);
...
gfil = '/global/work/pduarte/tmproms/run/run_Kongsfjorden-160m_present_subglacial/kongsfjorden_160m_grid_present.nc';
...
fsize = 30;  % Fontsize
figure('position',[5 50 70 500])
clf
plot(sbias(:,1),-zout,'b-','LineWidth',4); hold on;
plot(sbias(:,2),-zout,'r-','LineWidth',4); hold on;
plot(sbias(:,3),-zout,'m-','LineWidth',4); hold on;
legend('PSD','PTG0','PTG','Location','SouthWest');
for k=1:length(zout)
...
for k=1:length(zout)
  errors = MODSALT(:,k,3)-OBSSALT(:,k,3);                        % All model deviations from obs
  n = find(~isnan(errors)); errors = errors(n);                  % Remove NaNs
  err_neg = prctile(errors,10.); err_pos = prctile(errors,90.);  % Lower and upper value of error bar
  vals = [err_neg:0.01:err_pos];                                 % Make vector denoting the error bar
  plot(vals,repmat(-zout(k),length(vals)),'m-','LineWidth',2.0); hold on;
  plot([err_neg,err_pos],[-zout(k),-zout(k)],'m+','MarkerSize',7); hold on;
  clear errors n err_neg err_pos vals
end
...
plot(tbias(:,3),-zout,'m-','LineWidth',4); hold on;
legend('PSD','PTG0','PTG','Location','SouthWest');
for k=1:length(zout)
...
for k=1:length(zout)
  errors = MODTEMP(:,k,3)-OBSTEMP(:,k,3);                        % All model deviations from obs
  n = find(~isnan(errors)); errors = errors(n);                  % Remove NaNs
  err_neg = prctile(errors,10.); err_pos = prctile(errors,90.);  % Lower and upper value of error bar
  vals = [err_neg:0.01:err_pos];                                 % Make vector denoting the error bar
  plot(vals,repmat(-zout(k),length(vals)),'m-','LineWidth',2.0); hold on;
