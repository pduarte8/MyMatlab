sasa% Extract data for comparison with pyplume model
...
river_file = '/cluster/work/users/pduarte/tmproms/run/kongsfjorden_160m_river_present_subglacial.nc';
prename = 'ocean_avg';
%nfiles = 30;
%nfiles = 122;
nfiles = 90;
...
i1 = find(time >= date2unixsecs(2007,6,1)/(24*3600),1,'first');
i2 = find(time < date2unixsecs(2007,10,1)/(24*3600),1,'last');
[y,m,d] = datevec(time(i1:i2));

% Extract Q for main glaciers

Q = ncread('kongsfjorden_160m_river_present_subglacial.nc','river_transport');
Q_flux = transpose(Q);
...
hold on;
scatter(DisEnt_KoB(:,5),vf_KoB_sum/10,5.0,'o');
...
hold on;
scatter(DisEnt_KrB(:,5),vf_KrB_sum,5.0,'o');
...
hold on;
scatter(DisEnt_KoV(:,5),vf_KoV_sum,5.0,'o');
