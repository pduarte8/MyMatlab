...
A = load('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Edge_Q_fluxes.dat');
B = load('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Edge_Q_points_kongsfjorden_160m_present.txt');
...
gname = '/cluster/home/pduarte/models/metroms/apps/run_Kongsfjorden-160m_present_subglacial/kongsfjorden_160m_grid_present.nc';
%rdir = '/cluster/work/users/pduarte/tmproms/run/run_Kongsfjorden-160m_present_subglacial/Sim11';
%rdir = '/cluster/work/users/pduarte/tmproms/run/run_Kongsfjorden-160m_present_subglacial/Sim8';
%rdir = '/cluster/work/users/pduarte/tmproms/run/run_Kongsfjorden-160m_present_subglacial/Sim10';
%rdir = '/cluster/work/users/pduarte/tmproms/run/run_Kongsfjorden-160m_present_subglacial';
%rdir = '/cluster/work/users/pduarte/tmproms/run/run_Kongsfjorden-160m_present/Sim4';
rdir = '/tos-project3/NS9081K/NORSTORE_OSL_DISK/NS9081K/K160_bgc/Sim11/'
river_file = '/cluster/home/pduarte/models/metroms/apps/run_Kongsfjorden-160m_present_subglacial/kongsfjorden_160m_river_present_subglacial_plume.nc';
prename = strcat(rdir,'/ocean_avg_');
%nfiles = 30;
%nfiles = 122;
nfiles = 80;
...
river_time = ncread(river_file,'river_time');
i1 = find(river_time >= date2unixsecs(2007,6,1)/(24*3600),1,'first')
i2 = find(river_time < date2unixsecs(2007,10,1)/(24*3600),1,'last')
[y m d] = datevec(river_time(i1:i2));

% Extract Q for main glaciers

Q = ncread(river_file,'river_transport');
Q_flux = transpose(Q);
...
for fn=1:nfiles
  ...
  u = ncread(rfil,'u');
  v = ncread(rfil,'v');
  ubar = ncread(rfil,'ubar');
  vbar = ncread(rfil,'vbar');
...
% end
  ...
  ubar1_KoB(fn,:) = ubar(i_KoB-1,j_KoB,:);   ubar1_KoB(isnan(ubar1_KoB))=0;
  ubar2_KoB(fn,:) = ubar(i_KoB,j_KoB,:);     ubar2_KoB(isnan(ubar2_KoB))=0;
  vbar1_KoB(fn,:) = vbar(i_KoB,j_KoB-1,:);   vbar1_KoB(isnan(vbar1_KoB))=0;
  vbar2_KoB(fn,:) = vbar(i_KoB,j_KoB,:);     vbar2_KoB(isnan(vbar2_KoB))=0;  
...
DisEnt_KoB = csvread('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Kongsbreen_output_86m/kongsbreen_results_disch_ent_n1.csv',1,0);
figure
scatter(DisEnt_KoB(:,5),DisEnt_KoB(:,9),5.0,'o');
hold on;
size(DisEnt_KoB(:,5))
size(vf_KoB_sum)
scatter(DisEnt_KoB(1:nfiles,5),vf_KoB_sum,5.0,'o');
%scatter(DisEnt_KoB(:,5),vf_KoB_sum/12,5.0,'o');
legend('pyplume','ROMS','Location','NorthWest');
xlabel('Discharge volume flux');
ylabel('Entrainment volume flux');
title('Kongsbreen');

DisEnt_KrB = csvread('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Kronebreen_output/Kronebreen_results_disch_ent_nb.csv',1,0);
figure
scatter(DisEnt_KrB(:,5),DisEnt_KrB(:,9),5.0,'o');
hold on;
scatter(DisEnt_KrB(1:nfiles,5),vf_KrB_sum,5.0,'o');
%scatter(DisEnt_KrB(:,5),vf_KrB_sum/13,5.0,'o');
legend('pyplume','ROMS','Location','NorthWest');
xlabel('Discharge volume flux');
ylabel('Entrainment volume flux');
title('Kronebreen');

DisEnt_KoV = csvread('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Kongsvegen_output/Kongsvegen_results_disch_ent_nb.csv',1,0);
figure
scatter(DisEnt_KoV(:,5),DisEnt_KoV(:,9),5.0,'o');
hold on;
scatter(DisEnt_KoV(1:nfiles,5),vf_KoV_sum,5.0,'o');
%scatter(DisEnt_KoV(:,5),vf_KoV_sum/17,5.0,'o');
legend('pyplume','ROMS','Location','NorthWest');
xlabel('Discharge volume flux');
ylabel('Entrainment volume flux');
title('Kongsvegen');
...
% Load plume data
Zpp = csvread('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Kongsbreen_output_86m/kongsbreen_results_z.csv');
Tpp = csvread('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Kongsbreen_output_86m/kongsbreen_results_t_p.csv');
Spp = csvread('/cluster/home/pduarte/matlabmfiles/Model_animations/Tomas_scripts/pyplume_input/Kongsbreen_output_86m/kongsbreen_results_s_p.csv');
% for i=1:122
%     plot(Spp(i,:),Zpp(i,:)-Zpp(i,end),'b-',SALT_KoB(i,:),-z0_Kongsbreen,'r-',SALT_1(i,:),-z0_Kongsbreen_Ambient,'g-')
%     pause(0.25)
% end

%vf_KoB_surf = vf_KoB_sum;
%vf_KrB_surf = vf_KrB_sum;
%vf_KoV_surf = vf_KoV_sum;

vf_KoB_plm = vf_KoB_sum;
vf_KrB_plm = vf_KrB_sum;
vf_KoV_plm = vf_KoV_sum;

%save('VolumeFlux_surf.mat', 'vf_KoB_surf', 'vf_KrB_surf', 'vf_KoV_surf');
