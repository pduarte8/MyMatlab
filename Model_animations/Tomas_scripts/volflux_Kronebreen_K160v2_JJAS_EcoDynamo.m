...
%rdir = '/tos-project3/NS9081K/NORSTORE_OSL_DISK/NS9081K/K160_bgc/Sim4'
%rdir = '/tos-project3/NS9081K/NORSTORE_OSL_DISK/NS9081K/K160_bgc/Sim5'
%rdir = '/tos-project3/NS9081K/NORSTORE_OSL_DISK/NS9081K/K160_bgc/Sim6'
rdir = '/tos-project3/NS9081K/NORSTORE_OSL_DISK/NS9081K/K160_bgc/Sim13'
...
%     clear range
    ...
    phytoN_transect = zeros(length(X),N,nfiles);
    nh4_transect = zeros(length(X),N,nfiles);
...
%     VIN(i,:) = vin(X(i),Y(i),:);
  ...
  clear uin vin
  uin = UIN; vin = VIN;
  temp = TEMP; salt = SALT; no3 = NO3; phyto = PHYTO; phyto_N = PHYTO_N; nh4 = NH4;
  clear UIN VIN TEMP SALT NO3 PHYTO PHYTO_N
  ...
  if 0  % Plot to test that rotation of vectors is ok
    ...
    figure(5); clf; plot(u_along(:,end),'r-'); hold on;
                    plot(v_normal(:,end),'b-'); hold on; legend('u along','v normal');
                    line([1:length(X)],repmat(0,1,length(X)),'Color','k');   
    figure(6); clf; plot(u_along(:,end),'r-'); hold on;
                    plot(v_normal(:,end),'b-'); hold on; legend('u along','v normal');
                    line([1:length(X)],repmat(0,1,length(X)),'Color','k');    
    figure(7); clf; plot(u_along(:,end),'r-'); hold on;
                    plot(v_normal(:,end),'b-'); hold on; legend('u along','v normal');
                    line([1:length(X)],repmat(0,1,length(X)),'Color','k');                    
  ...
  phytoN_transect(:,:,fn) = phytoN_transect(:,:,fn) + phyto_N;
  nh4_transect(:,:,fn) = nh4_transect(:,:,fn) + nh4;
...
umean1 = mean(u_transect(:,:,:),3);
temp_mean = mean(temp_transect(:,:,:),3);
salt_mean = mean(salt_transect(:,:,:),3);
no3_mean = mean(no3_transect(:,:,:),3);
phyto_mean = mean(phyto_transect(:,:,:),3);
phytoN_mean = mean(phytoN_transect(:,:,:),3);
nh4_mean = mean(nh4_transect(:,:,:),3);
...
%--------------------
% Plot phytoplankton N
Vs = 0:0.1:1.0;
figure(6)
clf
%colormap(b)
[cs,hs] = contourf(dxtot/1000,-z0h,phytoN_mean,Vs, ...
    'LineWidth',0.05, 'LineStyle','none');
colormap(cm_haline);
%caxis([mincol maxcol]);
caxis([0 1]);
...
cbarrow;
%cb.Ticks = (mincol:(maxcol-mincol)/(length(b)-1):maxcol);
cb.Label.String = 'Phytoplankton ({\muM N})';
cb.Label.FontSize = 14;
%cb_ytick(cb,mincol:(maxcol-mincol)/(length(b)-1):maxcol,1);
%cb_title(cb,'cm/s',12);
plot(dxtot(:,1)/1000,-h,'k-','LineWidth',2); hold on;
...
ylabel('Depth (m)','FontSize',14,'FontWeight','bold');
set(gca,'FontSize',14,'FontWeight','bold');
print('-dpng','-r300','mean_phytoN_Kronebreen_K160v2_subglacial_JJAS_2007.png')
...
clf
%colormap(b)
[cs,hs] = contourf(dxtot/1000,-z0h,nh4_mean,Vs, ...
    'LineWidth',0.05, 'LineStyle','none');
colormap(cm_haline);
%caxis([mincol maxcol]);
caxis([0 3]);
...
ylabel('Depth (m)','FontSize',14,'FontWeight','bold');
set(gca,'FontSize',14,'FontWeight','bold');
print('-dpng','-r300','mean_nh4_Kronebreen_K160v2_subglacial_JJAS_2007.png')
...
clf
%colormap(b)
Nt = nh4_mean+no3_mean;
[cs,hs] = contourf(dxtot/1000,-z0h,Nt,Vs, ...
    'LineWidth',0.05, 'LineStyle','none');
colormap(cm_haline);
%caxis([mincol maxcol]);
caxis([0 8]);
...
cbarrow;
%cb.Ticks = (mincol:(maxcol-mincol)/(length(b)-1):maxcol);
cb.Label.String = 'Inorganic N ({\muM})';
cb.Label.FontSize = 14;
%cb_ytick(cb,mincol:(maxcol-mincol)/(length(b)-1):maxcol,1);
%cb_title(cb,'cm/s',12);
plot(dxtot(:,1)/1000,-h,'k-','LineWidth',2); hold on;
