...
Fig1 = figure('Position', [200 200 900 600]);
    i=20
    fname = strcat(dir_data,Filename)
            ...
            if ~isnan(mask(a,b)) && z_w(a,b,1) < depth_range(2)
                ...
                while (c < 36) && (z_w(a,b,c) < depth_range(2))
                    ...
                    if (a==1) || (b==1) || (a==xi) || (b==eta) ||...
                       length(find(u(a,b,:,i) > 10) > 0) || length(find(v(a,b,:,i) > 10) > 0) ||...
                       ((b > 0) && length(find(u(a,b-1,:,i) > 10) > 0)) ||...
                       ((a > 0) && length(find(v(a-1,b,:,i) > 10) > 0))
                        ubar(a,b,1) = NaN;
                        vbar(a,b,1) = NaN;
                        ubar(a,b,1) = NaN;
                        vbar(a,b,1) = NaN;
                    else
                        U = ( u(a,b-1,c,i) + u(a,b,c,i) ) / 2.0;
                        V = ( v(a-1,b,c,i) + v(a,b,c,i) ) / 2.0;
                        ubar(a,b,1) = ubar(a,b,1) +  ...
                            ( U*cos( theta(a,b) ) -  ...
                              V*sin( theta(a,b) ) ) * (z1 - z0);
                        vbar(a,b,1) = vbar(a,b,1) +  ...
                            ( U*sin( theta(a,b) ) +  ...
                              V*cos( theta(a,b) ) ) * (z1 - z0);
                    end
                    % Salinity and Temperature
                    sbar(a,b,1) = sbar(a,b,1) + salt(a,b,c,i) * (z1 - z0);
                    tbar(a,b,1) = tbar(a,b,1) + temp(a,b,c,i) * (z1 - z0);
                    % increment c
                    c = c+1;
                end
                % Average over depth
                ubar(a,b,1) = ubar(a,b,1) / h_layer;
                vbar(a,b,1) = vbar(a,b,1) / h_layer;
                sbar(a,b,1) = sbar(a,b,1) / h_layer;
                tbar(a,b,1) = tbar(a,b,1) / h_layer;                
            else
                ubar(a,b,1) = NaN;
                vbar(a,b,1) = NaN;
                sbar(a,b,1) = NaN;
...
%     CMH = colormap(cm_haline);
    %-- Temperature
    [CS,CH] = m_contourf(lon,lat,tbar(:,:,1), ...
        contour_tlines,'LineStyle','none');
    CMH = colormap(cm_thermal);
    %--
    qubar = [reshape(ubar(q1,q2,1),[1 qlen]) 0.1];
    qvbar = [reshape(vbar(q1,q2,1),[1 qlen]) 0.0];
