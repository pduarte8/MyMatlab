...
switch case_label
    case 'subglacial'
        ...
        % Data file directory
        dir_data = '/global/work/pduarte/tmproms/run/run_Kongsfjorden-160m_present_subglacial/Sim1/';
        % file list
    ...
    case 'future'
        ...
        % Data file directory
        dir_data = '/global/work/pduarte/tmproms/run/run_Kongsfjorden-160m_future/Sim2/StalloAndFramResults/';
        % file list
        file_list = sprintf('files_%s_%s_%s.txt', sim_label,case_label,month);
    case 'surface'
        % Grid file
        gname = '/global/work/pduarte/tmproms/run/run_Kongsfjorden-160m_present_subglacial/kongsfjorden_160m_grid_present.nc';
        % Data file directory
        dir_data = '/global/work/pduarte/tmproms/run/run_Kongsfjorden-160m_present/Sim3/';
        % file list
...
% Monthly mean values
%
red_ubar(:,:,1:59) = ubar(:,:,1:59);
red_ubar(:,:,60:79) = ubar(:,:,61:80);
ubar_mean = mean(red_ubar,3);
red_vbar(:,:,1:59) = vbar(:,:,1:59);
red_vbar(:,:,60:79) = vbar(:,:,61:80);
vbar_mean = mean(red_vbar,3);
red_sbar(:,:,1:59) = sbar(:,:,1:59);
red_sbar(:,:,60:79) = sbar(:,:,61:80);
sbar_mean = mean(red_sbar,3);
red_tbar(:,:,1:59) = tbar(:,:,1:59);
red_tbar(:,:,60:79) = tbar(:,:,61:80);
tbar_mean = mean(red_tbar,3);
red_NO3bar(:,:,1:59) = NO3bar(:,:,1:59);
red_NO3bar(:,:,60:79) = NO3bar(:,:,61:80);
NO3_mean = mean(red_NO3bar,3);
red_phytoplanktonbar(:,:,1:59) = phytoplanktonbar(:,:,1:59);
red_phytoplanktonbar(:,:,60:79) = phytoplanktonbar(:,:,61:80);
